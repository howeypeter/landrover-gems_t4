"""Display specs for the live-data gauges: scale range, redline, and preferred
widget style per parameter.

A gauge's *scale* (0–7000 rpm, a redline at 6000, etc.) is a presentation
decision, so it lives in the GUI layer rather than in ``gems/livedata`` — the
protocol/gems code stays free of display concerns. Screens look a spec up by the
parameter's local id; parameters without an explicit spec get a sensible dial
synthesised from their nominal value.
"""
from __future__ import annotations

from dataclasses import dataclass

from gems_t4.gems.livedata import PARAMETERS

# Widget styles a spec can request.
STYLE_DIAL = "dial"   # analog dial + needle (continuous numeric)
STYLE_BAR = "bar"     # horizontal bar (0..100 %-style)
STYLE_LCD = "lcd"     # amber numeric/enum readout (flags, enums)


@dataclass(frozen=True, slots=True)
class GaugeSpec:
    """How one live parameter is drawn as a gauge."""

    local_id: int
    label: str
    unit: str
    vmin: float
    vmax: float
    redline: float | None = None
    decimals: int = 0
    style: str = STYLE_DIAL


# Explicit specs for the main measures (local id -> spec).
_SPECS: tuple[GaugeSpec, ...] = (
    # Ids match livedata.PARAMETERS (real GEMS $21 ids for confirmed sensors;
    # emulator-only values in the 0x40+ block). Keep these in lockstep.
    GaugeSpec(0x00, "Coolant", "degC", -40, 120, redline=110, decimals=0),
    GaugeSpec(0x40, "Engine speed", "rpm", 0, 7000, redline=6000, decimals=0),
    GaugeSpec(0x41, "Battery", "V", 8, 16, redline=15, decimals=1),
    GaugeSpec(0x10, "Throttle", "%", 0, 100, decimals=0, style=STYLE_BAR),
    GaugeSpec(0x11, "Mass air flow", "kg/h", 0, 260, decimals=0),
    GaugeSpec(0x01, "Intake air", "degC", -20, 90, decimals=0),
    GaugeSpec(0x18, "O2 bank A", "V", 0.0, 1.0, decimals=2),
    GaugeSpec(0x42, "STFT", "%", -25, 25, decimals=0),
    GaugeSpec(0x43, "LTFT", "%", -25, 25, decimals=0),
    GaugeSpec(0x44, "IACV steps", "", 0, 200, decimals=0),
    GaugeSpec(0x45, "Ignition adv", "deg", -10, 45, decimals=0),
    GaugeSpec(0x46, "Road speed", "mph", 0, 120, decimals=0),
    GaugeSpec(0x47, "Loop status", "", 0, 2, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x48, "Misfire total", "", 0, 200, redline=50, decimals=0),
    GaugeSpec(0x15, "Fuel temp", "degC", -40, 120, decimals=0),
    GaugeSpec(0x49, "Idle target", "rpm", 0, 2000, decimals=0),
    GaugeSpec(0x4A, "Calc load", "%", 0, 100, decimals=0, style=STYLE_BAR),
    GaugeSpec(0x19, "O2 bank B", "V", 0.0, 1.0, decimals=2),
    GaugeSpec(0x4B, "Gearbox D/P", "", 0, 1, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x06, "A/C request", "", 0, 1, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x4C, "Ignition sw", "", 0, 1, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x4D, "Gbx retard", "%", 0, 100, decimals=0, style=STYLE_BAR),
    GaugeSpec(0x4E, "Inj pulse", "ms", 0, 20, redline=15, decimals=2),
    GaugeSpec(0x4F, "Coil charge", "ms", 0, 10, redline=8, decimals=2),
    GaugeSpec(0x50, "Security learn", "", 0, 1, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x51, "Mobilised", "", 0, 1, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x52, "Purge duty", "%", 0, 100, decimals=0, style=STYLE_BAR),
    GaugeSpec(0x53, "Fuel pump", "", 0, 1, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x54, "Run time", "s", 0, 65535, decimals=0, style=STYLE_LCD),
    # Per-cylinder misfire counters (V8, cylinders 1-8): small amber readouts
    # with a redline so a climbing counter reads at a glance on the grid.
    GaugeSpec(0x58, "Misfire cyl 1", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x59, "Misfire cyl 2", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x5A, "Misfire cyl 3", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x5B, "Misfire cyl 4", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x5C, "Misfire cyl 5", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x5D, "Misfire cyl 6", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x5E, "Misfire cyl 7", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
    GaugeSpec(0x5F, "Misfire cyl 8", "", 0, 255, redline=20, decimals=0, style=STYLE_LCD),
)

GAUGE_SPECS: dict[int, GaugeSpec] = {s.local_id: s for s in _SPECS}


def spec_for(local_id: int) -> GaugeSpec:
    """Return the gauge spec for a parameter, synthesising a dial if none exists."""
    spec = GAUGE_SPECS.get(local_id)
    if spec is not None:
        return spec
    p = PARAMETERS.get(local_id)
    if p is None:
        return GaugeSpec(local_id, f"id 0x{local_id:02X}", "", 0, 100)
    nominal = p.nominal if isinstance(p.nominal, (int, float)) else 0
    vmax = max(1.0, float(nominal) * 2.0)
    return GaugeSpec(local_id, p.name, p.unit, 0, vmax, decimals=0)


# ---- OBD-II Mode-01 PID gauge specs (real-ECU K-line profile) --------------- #
# Keyed by OBD PID, not stylized local id. Used by the live screen when the
# backend is on a real ECU (Backend.on_real_ecu).
_OBD_SPECS: dict[int, GaugeSpec] = {
    0x04: GaugeSpec(0x04, "Engine load", "%", 0, 100, decimals=0, style=STYLE_BAR),
    0x05: GaugeSpec(0x05, "Coolant", "degC", -40, 120, redline=110, decimals=0),
    0x06: GaugeSpec(0x06, "STFT B1", "%", -25, 25, decimals=1),
    0x07: GaugeSpec(0x07, "LTFT B1", "%", -25, 25, decimals=1),
    0x0C: GaugeSpec(0x0C, "Engine speed", "rpm", 0, 7000, redline=6000, decimals=0),
    0x0D: GaugeSpec(0x0D, "Vehicle speed", "km/h", 0, 200, decimals=0),
    0x0E: GaugeSpec(0x0E, "Timing adv", "deg", -10, 45, decimals=1),
    0x0F: GaugeSpec(0x0F, "Intake air", "degC", -40, 90, decimals=0),
    0x10: GaugeSpec(0x10, "MAF", "g/s", 0, 260, decimals=1),
    0x11: GaugeSpec(0x11, "Throttle", "%", 0, 100, decimals=0, style=STYLE_BAR),
    0x14: GaugeSpec(0x14, "O2 B1S1", "V", 0.0, 1.3, decimals=2),
    0x15: GaugeSpec(0x15, "O2 B1S2", "V", 0.0, 1.3, decimals=2),
}


def obd_spec_for(pid: int, name: str = "", unit: str = "") -> GaugeSpec:
    """Gauge spec for an OBD-II Mode-01 PID (real ECU K-line profile).

    Falls back to a plain dial synthesised from the measure's name/unit for
    PIDs without an explicit spec, so any supported PID still draws a gauge.
    """
    spec = _OBD_SPECS.get(pid)
    if spec is not None:
        return spec
    return GaugeSpec(pid, name or f"PID 0x{pid:02X}", unit, 0, 100, decimals=1)
