"""Tests for the ISO 9141-2 / OBD-II profile (gems_t4.protocol.obd).
...
class FakeObdEcu(Transport):
    ...
    def init(self, address: int, mode: str = "slow") -> InitResult:
        assert address == obd.OBD_INIT_ADDRESS
        return InitResult()
...
def test_encode_request_header_and_checksum() -> None:
    frame = obd.encode_request(b"\x01\x05")
...
def test_decode_response_strips_header_and_checksum() -> None:
    assert obd.decode_response(PID05) == bytes.fromhex("410500")


def test_decode_response_rejects_bad_checksum() -> None:
    bad = PID05[:-1] + bytes([0x00])
    with pytest.raises(obd.ObdError):
        obd.decode_response(bad)


def test_decode_response_rejects_bad_header() -> None:
    with pytest.raises(obd.ObdError):
        obd.decode_response(bytes.fromhex("00" + PID05.hex()[2:]))


def test_supported_pids_parses_bitmask() -> None:
    client = obd.ObdClient(FakeObdEcu(RESPONSES))
    client.connect()
    supported = client.supported_pids()
    # bf 9f f9 91 -> 01,03,04,05,06,07,08 in the first byte, etc.
...
def test_supported_pids_is_cached() -> None:
    ecu = FakeObdEcu(RESPONSES)
    client = obd.ObdClient(ecu)
    client.connect()
...
def test_read_dtcs_decodes_codes() -> None:
    ...
    resp += bytes([obd.obd_checksum(resp)])
    client = obd.ObdClient(FakeObdEcu({"03": resp}))
    client.connect()
...
def test_decode_dtcs_encoding(pair: str, code: str) -> None:
    assert obd.decode_dtcs(bytes.fromhex(pair)) == [code]


def test_decode_dtcs_skips_padding() -> None:
    assert obd.decode_dtcs(bytes.fromhex("03030000")) == ["P0303"]
